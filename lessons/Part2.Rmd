...
You have two dataframes: `d_pop` contains the state adult population in 2023, and `d_educ` contains the number of people with a BA and the number with a PhD. Join the dataframes and pivot them to create a dataframe with four columns: state, population, number of BA holders, number of PhD holders. Then find the state with the smallest difference in the share of people with BAs and PhDs.

⚠️ **Warning**: decide whether to use `join_full()`, `join_left()`, `join_right()` - will you get different results?

**💡 Tip:** it doesn't matter what order you do the join and pivot in - both orders will get you two the same final dataframe.

```{r}

# state level data on population
d_pop <- readRDS("../data/part2_data_join_1.rds")
dim(d_pop)

# state X education level data on population
d_educ <- readRDS("../data/part2_data_join_3.rds")
dim(d_pop)
...
\<Alt question: which state has the largest divergence from the state median?\>
...
## 🥊 Challenge Extra: pivot_longer() with .value
